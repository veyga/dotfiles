...
# Swagger ui preview (https://github.com/xavierchow/vim-swagger-preview)
function swagger_yaml2json() {
  ...
  if [[ $count -gt 2 ]]; then
    ...
    if grep -q "docker daemon running" $LOG; then
      ...
      docker run --rm -v $(pwd):/docs -v $TMP_DIR:/out openapitools/openapi-generator-cli generate -i /docs/"$1" -g openapi -o /out > $LOG 2>&1
      # clear the log file
      cp /dev/null $LOG
...
function swagger_ui_start() {
    ...
    if [ ! "$(docker ps -q -f name=$CONTAINER_NAME)" ]; then
        ...
        # run the container
        docker run --rm --name $CONTAINER_NAME -d -p 8017:8080 -e SWAGGER_JSON=/docs/openapi.json -v $TMP_DIR:/docs swaggerapi/swagger-ui
